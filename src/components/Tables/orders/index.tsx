...
import { Pencil, Eye, Search, X, Calendar, User, Package } from "lucide-react"
...
import { getReports, changeOrderStatus } from "@/app/actions"
...
export function Orders({ className }: { className?: string }) {
  ...
  const [filters, setFilters] = useState<FilterState>({
    dateFrom: new Date().toISOString().split("T")[0], // Current date in YYYY-MM-DD format
    dateTo: "",
    customer: "",
    product: "",
  })
  ...
  useEffect(() => {
    ...
    // Customer filter
    if (filters.customer) {
      filtered = filtered.filter((order) => order.customer_name.toLowerCase().includes(filters.customer.toLowerCase()))
    }

    // Product filter
    if (filters.product) {
      filtered = filtered.filter((order) =>
        order.items.some((item: OrderItem) => item.product_name.toLowerCase().includes(filters.product.toLowerCase())),
      )
    }
  ...
  // Get unique customers and products for filter suggestions
              ...
              <Card className="mb-4 sm:mx-4 xl:mx-8.5">
                <CardContent className="pt-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {/* Date From Filter */}
                    <div className="space-y-2 text-dark-3 font-bold">
                    ...
                    <div className="space-y-2 text-dark-3 font-bold">
                      ...
                      <Select value={filters.customer} onValueChange={(value) => handleFilterChange("customer", value)}>
                        <SelectTrigger className="text-dark-3 font-bold">
          ...
          <Table>
            ...
            <TableBody>
              {currentData.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={8} className="text-center py-8 text-black font-bold">
