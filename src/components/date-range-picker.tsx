  ...
  return (
    <div className={cn("grid gap-2", className)}>
      <Popover open={isPresetOpen} onOpenChange={setIsPresetOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            className={cn(
              ...
              !dateRange.from && "text-muted-foreground dark:!",
            )}
          >
        ...
        <PopoverContent 
          ref={presetPopoverContentRef} 
          className="w-[var(--radix-popover-trigger-width)] p-0 bg-white dark:!text-white dark:bg-gray-dark font-semibold" 
          align="start" // Align to the start of the trigger
        >
          <Command>
            <CommandList>
              <CommandEmpty></CommandEmpty>
              <CommandGroup className="w-full"> {/* Ensure CommandGroup takes full width of its parent */}
                {presets.map((preset) => (
